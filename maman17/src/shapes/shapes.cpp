...
unsigned int loadTexture(const char *filename)
{
    ...
    for (int i = 0; i < texSize; i++)
    {
        for (int j = 0; j < texSize; j++)
        {
            ...
            if ((x - 0.3f) * (x - 0.3f) + (y - 0.5f) * (y - 0.3f) < 0.005f || // Like
                (x - 0.8f) * (x - 0.8f) + (y - 0.6f) * (y - 0.7f) < 0.005f ||
                (x - 0.2f) * (x - 0.2f) + (y - 0.8f) * (y - 0.8f) < 0.005f ||
                (x - 0.45f) * (x - 0.45f) + (y - 0.92f) * (y - 0.92f) < 0.004f || // front top right
                (x - 0.7f) * (x - 0.7f) + (y - 0.2f) * (y - 0.2f) < 0.005f)       // front top left
            {
                texture[i][j][0] = 0;
                texture[i][j][1] = 0;
                texture[i][j][2] = 0;
